...
/**
 * Finds a segment with the BOOT flag set.
 * Returns NULL if no BOOT segment is found.
 */
static const struct Segment *find_boot_segment(void)
{
    for (int i = 0; i < g_segmentsCount; i++)
    {
        if (g_segments[i].flags & FLAG_BOOT)
            return &g_segments[i];
    }
    return NULL;
}
...
/**
 * Writes the entry.s file for the BOOT segment.
 * The entry code clears BSS and jumps to the boot function.
 */
static void write_entry_asm(FILE *fout, const struct Segment *boot_seg)
{
    const char *bss_seg_name = boot_seg->bss_seg;
    const char *entry_func = boot_seg->entry;
    const char *stack_expr = boot_seg->stack;

    if (bss_seg_name == NULL)
        util_fatal_error("BOOT segment '%s' missing 'boot_bss' directive", boot_seg->name);
    if (entry_func == NULL)
        util_fatal_error("BOOT segment '%s' missing 'entry' directive", boot_seg->name);
    if (stack_expr == NULL)
        util_fatal_error("BOOT segment '%s' missing 'stack' directive", boot_seg->name);

    fputs("/* Generated by mkldscript */\n\n"
          ".include \"macro.inc\"\n\n"
          ".set noat\n"
          ".set noreorder\n\n"
          ".section .text, \"ax\"\n\n"
    ...
    // Load BSS start address
    fprintf(fout, "    lui     $t0, %%hi(_%sSegmentBssStart)\n", bss_seg_name);
    fprintf(fout, "    addiu   $t0, $t0, %%lo(_%sSegmentBssStart)\n", bss_seg_name);

    // Load BSS size
    fprintf(fout, "    lui     $t1, %%hi(_%sSegmentBssSize)\n", bss_seg_name);
    fprintf(fout, "    addiu   $t1, $t1, %%lo(_%sSegmentBssSize)\n", bss_seg_name);
    ...
    // Load entry function address and jump
    fprintf(fout, "    lui     $t2, %%hi(%s)\n", entry_func);
    fprintf(fout, "    addiu   $t2, $t2, %%lo(%s)\n", entry_func);

    // Set up stack pointer and jump to entry
    fprintf(fout, "    lui     $sp, %%hi(%s)\n", stack_expr);
    fputs("    jr      $t2\n", fout);
    fprintf(fout, "    addiu   $sp, $sp, %%lo(%s)\n\n", stack_expr);

    // Pad to 0x50 bytes
    fputs(".fill 0x50 - (. - entrypoint)\n", fout);
}

static void usage(const char *execname)
{
    fprintf(stderr, "Nintendo 64 linker script generation tool v0.04\n"
                    "usage: %s SPEC_FILE LD_SCRIPT [ENTRY_ASM]\n"
                    "SPEC_FILE  file describing the organization of object files into segments\n"
                    "LD_SCRIPT  filename of output linker script\n"
                    "ENTRY_ASM  optional filename for generated entry.s (for BOOT segments)\n",
                    execname);
}
...
int main(int argc, char **argv)
{
    ...
    void *spec;
    size_t size;
    const char *entry_asm_path = NULL;
    ...
    // Write entry.s if requested and a BOOT segment exists
    if (entry_asm_path != NULL)
    {
        const struct Segment *boot_seg = find_boot_segment();
        if (boot_seg != NULL)
        {
            entryout = fopen(entry_asm_path, "w");
            if (entryout == NULL)
                util_fatal_error("failed to open file '%s' for writing", entry_asm_path);
            write_entry_asm(entryout, boot_seg);
            fclose(entryout);
        }
    }
