...
static char* expand_variables(const char* input) {
    ...
    while (*p) {
        if (p[0] == '$' && p[1] == '(') {
            ...
            if (varEnd) {
                size_t varLen = varEnd - varStart;
                char varName[256];
                if (varLen < sizeof(varName)) {
                    memcpy(varName, varStart, varLen);
                    varName[varLen] = '\0';
                    const char* value = getenv(varName);
                    if (value) {
                        outSize += strlen(value);
                    }
                    // else variable not found, omit it
                }
                p = varEnd + 1;
                continue;
            }
...
static const char* const stmtNames[] = {
    ...
    [STMT_beginwave] = "beginwave",
    [STMT_endwave] = "endwave",
};
// clang-format on
...
bool parse_segment_statement(struct Segment* currSeg, STMTId stmt, char* args, int lineNum) {
    ...
    switch (stmt) {
        ...
        case STMT_include: {
            ...
            char* rawPath;
            if (!parse_quoted_string(args, &rawPath))
                util_fatal_error("line %i: invalid filename", lineNum);

            // Expand $(VAR) environment variables in the path
            currSeg->includes[currSeg->includesCount - 1].fpath = expand_variables(rawPath);
...
void parse_rom_spec(char* spec, struct Segment** segments, int* segment_count) {
    ...
    while (line[0] != 0) {
        ...
        if (line[0] != 0 && stmtName[0] != 0) {
            ...
            // Skip wave sections entirely (used for ELF debugging, not needed for linker script)
            if (stmt == STMT_beginwave) {
                insideWave = true;
                line = nextLine;
                lineNum++;
                continue;
            }
            if (stmt == STMT_endwave) {
                insideWave = false;
                line = nextLine;
                lineNum++;
                continue;
