#include "common.h"


INCLUDE_ASM(const s32, "lib/os/libultra/libnaudio/n_reverb", func_800FA630);

INCLUDE_ASM(const s32, "lib/os/libultra/libnaudio/n_reverb", func_800FA940);

INCLUDE_ASM(const s32, "lib/os/libultra/libnaudio/n_reverb", _n_loadOutputBuffer);

INCLUDE_ASM(const s32, "lib/os/libultra/libnaudio/n_reverb", n_reverb_text_06E0);

INCLUDE_ASM(const s32, "lib/os/libultra/libnaudio/n_reverb", _n_loadBuffer);

INCLUDE_ASM(const s32, "lib/os/libultra/libnaudio/n_reverb", n_reverb_text_0800);

INCLUDE_ASM(const s32, "lib/os/libultra/libnaudio/n_reverb", _n_saveBuffer);

INCLUDE_ASM(const s32, "lib/os/libultra/libnaudio/n_reverb", n_reverb_text_0918);

INCLUDE_ASM(const s32, "lib/os/libultra/libnaudio/n_reverb", _n_filterBuffer);

INCLUDE_ASM(const s32, "lib/os/libultra/libnaudio/n_reverb", func_800FB020);
